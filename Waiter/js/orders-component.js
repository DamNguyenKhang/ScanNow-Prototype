const OrdersComponent = {
    ...
    render: function (container, params) {
        if (params.table) this.activeTableFilter = params.table;

        container.innerHTML = `
            <!-- FILTER TABS (Sticky) -->
            <div class="sticky top-0 z-30 bg-white border-b border-border-main h-[44px] shrink-0">
            ...
            <main class="flex-1 overflow-y-auto pb-24 pt-3 no-scrollbar h-full">
    ...
    renderOrders: function (listEl, emptyEl) {
        ...
        listEl.innerHTML = filtered.map(o => {
            ...
            const isUrgent = diffMin >= 10 && o.status !== 'completed';
            const itemsSummary = o.items.map(i => `${i.name} ×${i.qty}`).join(', ');
            const total = o.items.reduce((acc, curr) => acc + (curr.price * curr.qty), 0);

            const accentColor = o.status === 'pending' ? 'status-pending' : o.status === 'preparing' ? 'primary-orange' : o.status === 'ready' ? 'status-ready' : 'slate-200';
                    ...
                    <div class="p-4 pt-5 pb-4">
                        ...
                        <div class="flex justify-between items-end border-t border-slate-50 pt-3">
                            <div>
                                ...
                                <span class="font-mono font-bold text-[16px] text-primary-orange">${formatCurrency(total)}</span>
                            </div>
                            ${o.status === 'pending'
                    ? `<button onclick="OrdersComponent.cancelOrder('${o.id}', event)" class="h-9 px-4 bg-red-50 text-red-500 text-[13px] font-bold rounded-[10px] active:bg-red-100 transition-colors flex items-center gap-1">
    ...
    updateStatus: function (id, status, e) {
        // Restricted but keeping for internal demo/dev logic or later expansion
        if (e) e.stopPropagation();
    ...
    openOrderDetails: function (id) {
        ...
        const total = Math.floor(subtotal * 1.08) + 5000;

        content.innerHTML = `
            <div class="px-4 pb-4 border-b border-border-main relative shrink-0">
                <div class="flex justify-between items-start mb-1"><span class="font-mono text-[13px] text-text-muted">${o.id}</span>
                <div class="flex items-center gap-2">
                    ${o.status === 'pending' ? `<button onclick="router.navigate('create-order', {edit: '${o.id}'})" class="size-8 flex items-center justify-center text-primary bg-primary/5 rounded-full"><span class="material-symbols-outlined text-[20px]">edit</span></button>` : ''}
            ...
            <div class="flex-1 overflow-y-auto p-4 no-scrollbar">
                <div class="mt-2 space-y-3">
                    ${o.items.map((item, idx) => `
                        <div class="flex items-center gap-3 py-3 border-b border-[#F1EFE9]">
                            ...
                            <div class="flex-1">
                                <div class="flex justify-between items-start"><span class="font-display font-bold text-[14px] text-text-main">${item.name}</span><div class="text-right"><span class="text-[13px] text-text-sub mr-2">×${item.qty}</span></div></div>
                            </div>
                        </div>
                    `).join('')}
                </div>
                ...
                <div class="mt-8 space-y-2.5">
                    <div class="flex justify-between items-center text-[14px] text-text-sub"><span>Tạm tính</span><span class="font-mono text-text-main">${formatCurrency(subtotal)}</span></div>
                    <div class="flex justify-between items-center text-[14px] text-text-sub"><span>Phí phục vụ + VAT (8%)</span><span class="font-mono text-text-main">${formatCurrency(total - subtotal)}</span></div>
                    <div class="border-t border-border-main pt-3 mt-3 flex justify-between items-center"><span class="font-bold">Tổng cộng</span><span class="font-mono font-bold text-[20px] text-primary">${formatCurrency(total)}</span></div>
                </div>
            </div>
            <div class="p-4 border-t border-border-main bg-white shrink-0 pb-safe">
                ${o.status === 'pending' ? `<button onclick="OrdersComponent.cancelOrder('${o.id}')" class="w-full h-12 bg-white border border-red-500 text-red-500 font-bold rounded-xl transition-colors active:bg-red-50">Hủy đơn hàng</button>` : `<div class="w-full h-12 flex items-center justify-center bg-slate-50 text-text-muted font-medium rounded-xl gap-2"><span class="material-symbols-outlined text-[20px]">sync</span> ${o.status === 'preparing' ? 'Đang chế biến...' : o.status === 'ready' ? 'Đã sẵn sàng phục vụ' : 'Đã hoàn thành'}</div>`}
            </div>
        `;
